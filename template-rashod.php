<?php get_header();
/* Template Name: Опции и расходные материалы*/
?>
		...
		<div class="wrap1 main_wrap sidebar consumables">
			...
			<div class="sidebar__left sidebar-left">
				<ul class="sidebar-left__list">
					...
					<li class="sidebar-left__item accordion sidebar-left__item--product">
						<h2 class="sidebar-left__title sidebar-left__title--drop-down accordion__control f-18  ">
							<a class="sidebar-left__link">Опции и расходные материалы</a>
						</h2>

						<ul class="sidebar-left__inner-list consumables-menu accordion__content">
							<?php
							$prodRepeat = get_field('options_filtr_elements');

							if ($prodRepeat) :
								foreach ($prodRepeat as $item) : ?>
									<li class="consumables-menu__item accordion-inner">
										<h3 class="consumables-menu__title f-16 fb accordion-inner__control">
											<a class="consumables-menu__link"><?= $item['options_filtr_elements_title']; ?></a>
										</h3>

										<ul class="consumables-menu__inner-list accordion-inner__content">
											<?php
											if ($item['options_filtr_elements_elements']) :
												foreach ($item['options_filtr_elements_elements'] as $i) : ?>
													<li class="consumables-menu__inner-item">
														<a class="consumables-menu__inner-link">
															<span class="consumables-menu__checkbox"><?= $i['options_filtr_elements_elements_title']; ?></span>
														</a>
													</li>
											<?php endforeach;
											endif; ?>
				...
				<div class="consumables__device-search device-search bra bg-fff mt30">
					...
					<button class="device-search__button mt30" type='submit'>ок</button>
				</div>
			</div>
